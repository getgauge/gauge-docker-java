...
var BUILD_DIR_GAUGE_JAVA = filepath.Join(BUILD_DIR_SRC, dockerJava)
var platformBinDir = filepath.Join(bin, fmt.Sprintf("%s_%s", runtime.GOOS, runtime.GOARCH))
var deployDir = filepath.Join(deploy, dockerJava)

var goFiles = []string{"gauge-docker-java.go"}
...
func copyGoFilesToGoPath() {
	for _, f := range goFiles {
		mirrorFile(f, filepath.Join(BUILD_DIR_SRC, dockerJava, f))
	}
}
...
func copyGoFiles(destDir string) {
	files := make(map[string]string)
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), dockerJava+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), dockerJava)] = bin
	}

	files[filepath.Join("Dockerfile")] = ""
	files[filepath.Join("docker-java.json")] = ""
	files[filepath.Join("skel", "StepImplementation.java")] = filepath.Join("skel")
	files[filepath.Join("skel", "docker-java.properties")] = filepath.Join("skel", "env")
...
func getDockerJavaVersion() string {
	javaRunnerProperties, err := getPluginProperties("docker-java.json")
	if err != nil {
		panic(fmt.Sprintf("Failed to get docker-java properties file. %s", err))
	}
...
func main() {
	...
	if *install {
		...
		installDockerJava(*pluginInstallPrefix)
	} else if *distro {
		createGaugeDistro(*allPlatforms)
	} else if *test {
		compileGoPackage(dockerJava)
	} else {
		compileDockerJava()
	}
}

func compileDockerJava() {
	if *allPlatforms {
		compileDockerJavaAcrossPlatforms()
	} else {
		compileGoPackage(dockerJava)
	}
	copyBinaries()
	moveOSBinaryToCurrentOSArchDirectory(dockerJava)
}
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", dockerJava, getDockerJavaVersion(), getGOOS(), getArch())
	distroDir := filepath.Join(deploy, packageName)
	copyGoFiles(distroDir)
...
func compileDockerJavaAcrossPlatforms() {
	for _, platformEnv := range platformEnvs {
		setEnv(platformEnv)
...
func installDockerJava(installPrefix string) {
	os.RemoveAll(deployDir)
	copyGoFiles(deployDir)
	javaRunnerInstallPath := filepath.Join(installPrefix, "docker-java", getDockerJavaVersion())
